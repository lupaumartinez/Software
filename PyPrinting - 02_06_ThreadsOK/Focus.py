...
class Backend(QtCore.QObject):
    ...
    autofinishSignal = pyqtSignal()   #mode_printing
    autofinishSignal_dimers = pyqtSignal()
    ...
    def ramp_lin(self, laser):
        ...
        PDtask = channels_photodiodos(nidaqmx, self.frequency, self.Ntot_read)

        channels_triggers(PDtask, 'Z')

        self.pi_device.TWC() 
        self.pi_device.CTO(3, 3, 3)
        self.pi_device.CTO(3, 5, self.zo + self.extra)
        self.pi_device.CTO(3, 6, self.zo + self.range_total - self.extra)

        nciclos = 1
        self.pi_device.WGC(3, nciclos)
        self.pi_device.WTR(3, self.WTRtime, 0)
             
        self.pi_device.WAV_LIN(3, 0, self.Npoints, "X", self.Nspeed, self.range_total, 0, self.Npoints)
        self.pi_device.WAV_LIN(3, 0, self.Npoints, "&", self.Nspeed, -self.range_total, self.range_total, self.Npoints)
        
        self.pi_device.WGO(3, True) 
        data_read = PDtask.read(self.Ntot_read)
        PDtask.close()

        #data_ph = np.array(data_read[PD_channels[laser]])
    ...
    def move_z(self, dist):
        ...
        self.pi_device.MOV('C', dist)
        #print("me muevo a z", dist)
        
        while not all(self.pi_device.qONT('C').values()):
            time.sleep(0.1)
    ...
    def focus_autocorr_lin_x2(self, mode_printing):
        ...
        if mode_printing == 'printing':
           self.autofinishSignal.emit()
               
        if mode_printing == 'dimers':
           self.autofinishSignal_dimers.emit()
