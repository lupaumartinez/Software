...
from Instrument import shutters, Flipper_notch532, pi_device
...
# Reference           
    ...
    def grid_finish_autofoco(self):
        ...
        if self.shiftx != 0  or self.shifty != 0 :
           ...
           pi_device.MOV(axes, targets_return)
           time.sleep(0.01)
    ...
    def grid_center_scan_detect(self, image, center_mass):
        
        self.save_scan(image)
    ...
    def grid_confocal_spectrum(self):
        ...
        folder_NP_spectrum = os.path.join(self.new_folder, "Confocal_Spectrum_" + Col + "_" + NP)
        os.makedirs(folder_NP_spectrum)
        
        self.grid_spectrumSignal.emit(self.laser, self.mode_printing, folder_NP_spectrum) 
            
    @pyqtSlot()                
    def grid_confocal_spectrum_detect(self): 
    ...
    def save_scan(self, image):
        ...
        f = open(name, "w")
        guardadoida = Image.fromarray(np.transpose(image))
        guardadoida.save(name)  
        f.close()
        print("\n Center scan saved\n")   
...
if __name__ == '__main__':
    ...
    printingThread = QtCore.QThread()
    worker.moveToThread(printingThread)
