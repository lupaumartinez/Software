...
# 'cyclic', 'greyclip', 'grey' # Solo son estos
        ...
        viewDock = Dock('Viewbox',size=(100,40))
...
class Backend(QtCore.QObject):
   ...
   # aspectSignal = pyqtSignal(int)
        ...
        if self.mode_printing == 'none':
            ...
            if self.scan_mode_option == Scanmodes[0]:
                if self.psf_mode_option == PSFmodes[0]:
                    ...
                    pi_device.MOV(['A','C'], [self.x_pos, self.z_pos])
                elif self.psf_mode_option == PSFmodes[2]:
                    self.PDtimer_rampyx.stop()   
                    pi_device.MOV(['B','A'], [self.y_pos, self.x_pos])
                elif self.psf_mode_option == PSFmodes[3]:
                    self.PDtimer_rampyz.stop()   
                    pi_device.MOV(['B','C'], [self.y_pos, self.z_pos])
    ...
    def scan_step_parameters(self, parameters): 
       ...
       # aspect = int(self.range_y/self.range_x)
    ...
    def scan_step_xy(self):   
        ...
        if self.j < self.Ny:
            if self.i < self.Nx :
                pi_device.MOV(['A', 'B'], [self.matrix_scan_step[0][self.i], self.matrix_scan_step[1][self.j]])    
    ...
    def scan_step(self):
        ...
        PDtask = channels_photodiodos(self.rate, self.Nph) 
    ...
    def scan_ramp_xy(self):
        ...
        if self.i < self.Ny: 
           ...
           pi_device.MOV('B', self.y_pos - self.range_y/2 + dy/2 + self.i*dy) 
    ...
    def scan_ramp_xz(self):
        ...
        if self.i < self.Ny: 
           ...
           pi_device.MOV('C', self.z_pos - self.range_y/2 + dz/2 + self.i*dz) 
    ...
    def scan_ramp_yx(self):
        ...
        if self.i < self.Nx: 
           ...
           pi_device.MOV('A', self.x_pos - self.range_x/2 + dx/2 + self.i*dx) 
    ...
    def scan_ramp_yz(self):
        ...
        if self.i < self.Nx: 
           ...
           pi_device.MOV('C', self.z_pos - self.range_x/2 + dz/2 + self.i*dz) 
    ...
    def scan_ramp_x_lin_configuration(self, x_pos):
        ...
        pi_device.WAV_LIN(1, 0, Npoints, "X", Nspeed, self.range_total, 0, Npoints)
        pi_device.WAV_LIN(1, 0, Npoints, "&", Nspeed, -self.range_total, self.range_total, Npoints)
        ...
        pi_device.TWC() 
        pi_device.CTO(1, 3, 3)
        pi_device.CTO(1, 5, xo + self.extra)
        pi_device.CTO(1, 6, xo + self.range_total - self.extra)
    ...
    def scan_ramp_y_lin_configuration(self, x_pos):
        ...
        #opcion 2 lineales ida y vuelta:
        pi_device.WAV_LIN(2, 0, Npoints, "X", Nspeed, self.range_total_y, 0, Npoints)
        pi_device.WAV_LIN(2, 0, Npoints, "&", Nspeed, -self.range_total_y, self.range_total_y, Npoints)
        ...
        pi_device.TWC() 
        pi_device.CTO(2, 3, 3)
        pi_device.CTO(2, 5, xo + self.extra_y)
        pi_device.CTO(2, 6, xo + self.range_total_y - self.extra_y)
