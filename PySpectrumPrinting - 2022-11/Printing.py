...
class Frontend(QtGui.QFrame):
    ...
    parametersSignal = pyqtSignal(int, list, bool)
    ...
    # crea la carpeta vacía
        self.imprimir_button = QtGui.QPushButton('PRINTING folder')
    ...
    # Indice actual
        ...
        grid_print_layout.addWidget(self.scan_check,              5, 0)
        grid_print_layout.addWidget(self.set_ref_button,          6, 0)
        grid_print_layout.addWidget(self.play_button,             7, 0)
        grid_print_layout.addWidget(self.pause_button,            8, 0)
        grid_print_layout.addWidget(self.next_button,             9, 0)
 ...
 #extra info:
        ...
        self.NPevents = QtGui.QLineEdit('Nan')
        self.NPsuccess = QtGui.QLineEdit('Nan')
        ...
        grid_info.addWidget(self.NPevents,                4, 1)
        grid_info.addWidget(self.NPsuccess,               5, 1)
        grid_info.addWidget(self.extra_info,              6, 1)
        grid_info.addWidget(self.grid_save_info_button,   7, 1)
        ...
        grid_info.addWidget(QtGui.QLabel('Events printing (%)'),          4, 0)
        grid_info.addWidget(QtGui.QLabel('Success events (%)'),           5, 0)
        grid_info.addWidget(QtGui.QLabel('Extra comments'),               6, 0)
    ...
    def get_grid_parameters(self):
        ...
        scanbool = self.scan_change()
           
        self.parametersSignal.emit(color_laser, grid_parameters, scanbool)
    ...
    @pyqtSlot(list)
    def plot_printing_error(self, printing_error):

        xerror = printing_error[0]
        yerror = printing_error[1]
        
        self.error_widget = pg.GraphicsLayoutWidget()
        ploterror = self.error_widget.addPlot(row=2, col=2, title="Error printing")
        ploterror.showGrid(x=True, y=True)
        ploterror.setLabel('left', "Error x (nm)")
        ploterror.setLabel('bottom', "Error y (nm)")
        
        subgrid = QtGui.QGridLayout()
        subgrid.addWidget(self.error_widget)
               
        curve = ploterror.plot(open='y')
        curve.setData(yerror, xerror, pen=None, symbol='o')
    ...
    def get_grid_info(self):
        ...
        info = [["Laser:", str(self.grid_laser.currentText()) ], ["Umbral: ", str(self.umbralEdit.text())], ["Umbral down: ", str(self.umbral_downEdit.text())],
                ...
                ["NPs: ", str(self.typeNP.text())], ["Substrate: ", str(self.substrate.text())],
                 ["Total Events on printing (%): ", str(self.NPevents.text())], 
                  ["Success events (%): ", str(self.NPsuccess.text())], 
    ...
    def make_connection(self, backend):
        ...
        backend.printingerrorSignal.connect(self.plot_printing_error)      
...
class Backend(QtCore.QObject):
    ...
    printingerrorSignal = pyqtSignal(list)
    ...
    def __init__(self, pi_device, task_nidaqmx, *args, **kwargs):
        ...
        self.printing_error_vector_x = []
        self.printing_error_vector_y = []
...
# Reference           
    ...
    def grid_create_folder(self):
        ...
        try:
            print("Grid laod")
            new_folder = self.old_folder + "/" + timestr + "_Printing_" + self.grid_name
        except IOError as e:
    ...
    def grid_parameters(self, color_laser, grid_parameters, scanbool):
    ...
    def grid_trace_detect(self, data):  
        ...
        if I_new > I_old*self.umbral or I_new < I_old*self.umbral_down or (ptime.time() - self.timer_inicio) > self.timemax:
               ...
               print('printing time trace', self.timer_real)
               self.save_trace()
    ...
    def grid_scan_detect(self, image, center_mass, image_gone, image_back):
        
        self.save_scan(image, image_gone, image_back)
        self.grid_printing_error(center_mass)
    ...
    def grid_printing_error(self, center_mass):
       
        target = [self.grid_x[self.i_global] + self.startX,
                   self.grid_y[self.i_global] + self.startY]      
   
        printing_error_x = (target[0] - center_mass[0])*10**3  #nm
        printing_error_y = (target[1] - center_mass[1])*10**3  #nm   
        
        self.printing_error_vector_x.append(printing_error_x)
        self.printing_error_vector_y.append(printing_error_y)

       # self.printingerrorSignal.emit([ self.printing_error_vector_x, self.printing_error_vector_y])
        self.grid_printing_error_save()
        
    def grid_printing_error_save(self): 
        
        filepath = self.new_folder
        timestr = time.strftime("%Y%m%d-%H%M%S")
        name = str(filepath + "/" + "printing_error-" + timestr + ".txt")
        
        f = open(name, "w")
        np.savetxt(name, np.transpose([self.printing_error_vector_x, self.printing_error_vector_y]), fmt='%.3f')
        f.close()
        print("\n Printing error saved\n")
