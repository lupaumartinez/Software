    ...
    def scan_step_xy(self):   
        ...
        if self.j < self.Ny:
            ...
            if self.i < self.Nx :
                ...
                self.pi_device.MOV(['A', 'B'], [self.x_pos + self.i*dx, self.y_pos + self.j*dy]) 
...
#            center_mass = [x_cm, y_cm]
            ...
            if self.mode_printing == 'luminescence':
                self.saveFrame()
                self.scanfinishSignal_luminescence.emit([self.x_pos, self.y_pos])
    ...
    def create_folder(self, time):
        ...
        new_folder = os.path.join(self.old_folder, time + "_Fast_Confocal_Spectrum")       
    ...
    def saveFrame(self):
        ...
        filepath = self.new_folder
        timestr = time.strftime("%Y%m%d-%H%M%S")
        name = str(filepath + "/" + "Fast_Confocal_Spectrum" + timestr + ".tiff")
