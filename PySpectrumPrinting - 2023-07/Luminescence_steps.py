...
class Frontend(QtGui.QFrame):
    ...
    # Opcion de No prender el laser durante la medicion, para Scattering
        laser_on_Label = QtGui.QLabel('Modo Scattering, laser apagado')
        self.laser_on_auto = QtGui.QCheckBox('')
        self.laser_on_auto.clicked.connect(self.get_grid_laser_on)
        self.laser_on_auto.setToolTip('Si esta marcado, no prende el laser para la medicion, solo lo usa para centrar.')
        
    # crea la carpeta vacía
        self.imprimir_button = QtGui.QPushButton('Luminescence Steps folder')
    ...
    # Indice actual
        ...
        grid_print_layout.addWidget(center_scan_Label,            2, 0)
        grid_print_layout.addWidget(self.center_scan_auto,        2, 1)
        
        grid_print_layout.addWidget(laser_on_Label,               3, 0)
        grid_print_layout.addWidget(self.laser_on_auto,           3, 1)
 ...
 #extra info:
    ...
    def get_grid_laser_on(self):
        
        if self.laser_on_auto.isChecked():
            self.laseronSignal.emit(True)
        else:
            self.laseronSignal.emit(False)
...
class Backend(QtCore.QObject):
    ...
    def __init__(self, pi_device, task_nidaqmx, *args, **kwargs):
        ...
        self.laser_on_option  = True  #Medir con laser prendido, apagar para Scattering
...
# Reference           
    ...
    def grid_create_folder(self):
        ...
        try:
            print("Grid load")
            new_folder = self.old_folder + "/" + timestr + "_Luminescence_Steps_" + self.grid_name
        except IOError as e:
    ...
    @pyqtSlot(bool)
    def grid_laser_on_option(self, bool_laser_on):
        
        if bool_laser_on:
            self.laser_on_option = False
        else:
            self.laser_on_option = True
    ...
    def grid_start(self):
        ...
        self.mode_printing = 'luminescence_steps'
           
        self.startX = self.xref
        self.startY = self.yref
       # self.startZ = self.zref
    ...
    def grid_move(self):
        ...
        targets = [self.grid_x[self.i_global] + self.startX,
                   self.grid_y[self.i_global] + self.startY]#, self.grid_z[self.i_global] + self.startZ]
    ...
    def grid_autofoco(self):
        ...
        if self.i_global in multifoco:
            ...
            if self.center_scan_option:
                ...
                self.grid_step_and_glue_spectrum()
    ...
    def make_connection(self, frontend):
        ...
        frontend.laseronSignal.connect(self.grid_laser_on_option)
