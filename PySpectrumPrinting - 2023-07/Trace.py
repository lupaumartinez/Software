...
import nidaqmx
from Instrument_nidaqmx import initial_nidaqmx, channels_photodiodos, shutters, openShutter, closeShutter, PDchans, PD_channels, rateNI
...
class Backend(QtCore.QObject):
    ...
    def __init__(self, task_nidaqmx, *args, **kwargs):
        ...
        self.plot_pointtimer = QtCore.QTimer() 
        
        self.plot_subdata = [0, 0, 0, 0, 0, 0, 0]
    ...
    def trace_configuration(self, laser, mode_printing): 
        ...
        self.PDtask = channels_photodiodos(nidaqmx, self.ratePDchannel, self.N)
        
        self.pointtimer.start(0) #mide lo mas rápido que puede
        self.plot_pointtimer.start(0)  #cada 50 ms actualiza el plot
             
    def trace_update(self):     
        
        tic = ptime.time()
        
        lectura_total = self.PDtask.read(self.N)

        self.points = lectura_total[PDchans.index(PD_channels[self.laser])]
        m = round(np.mean(self.points), 5)
        self.data1.append(m)
        
        self.points_BS = lectura_total[PDchans.index(PD_channels['BS'])]
        m_BS = round(np.mean(self.points_BS), 5)
       ...
       # M2 = 10 # second window or before event
        ...
        self.step_time = round(self.step_time + tiempo, 6)
    ...
    def plot_trace_update(self):
        
        show = 800
        if self.ptr < show:
            time = self.timeaxis
            intensity = self.data1
            intensity_BS = self.data_BS
        else:
            time = self.timeaxis[-show:]
            intensity = self.data1[-show:]
            intensity_BS = self.data_BS[-show:]
        
        self.plot_subdata = [self.ptr, time, intensity, self.mediochico2, self.mediochico, intensity_BS, self.mean_BS]
        
        self.dataSignal.emit(self.plot_subdata)
...
if __name__ == '__main__':
    ...
    worker.moveToThread(traceThread)
    worker.pointtimer.moveToThread(traceThread)
    worker.pointtimer.timeout.connect(worker.trace_update)
    worker.plot_pointtimer.moveToThread(traceThread)
    worker.plot_pointtimer.timeout.connect(worker.plot_trace_update)
    traceThread.start()
