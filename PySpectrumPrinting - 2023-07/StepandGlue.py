...
class Frontend(QtGui.QFrame):
    ...
    signal_lock_Signal =  pyqtSignal(float, float)
    signal_lock_ON_Signal =  pyqtSignal(bool)
    ...
    def setUpGUI(self):
        ...
        # Lockear la señal del sustrato
        self.signal_lock_button = QtGui.QPushButton('Lock Signal on Sustrate')
        self.signal_lock_button.setCheckable(True)
        self.signal_lock_button.clicked.connect(self.get_lock_signal)
        self.signal_lock_button.setToolTip('guarda una señal para restar luego, lo uso para restar la señal del sustrato')
        self.signal_lock_button.setStyleSheet(
                "QPushButton { background-color: orange; }"
                "QPushButton:pressed { background-color: blue; }")
        
        self.signal_lock_ON_button = QtGui.QCheckBox('')
        self.signal_lock_ON_button.clicked.connect(self.get_lock_signal_ON)
        self.signal_lock_ON_button.setToolTip('si esta check, resta la señal del sustrato')
        ...
        parameters_layout.addWidget(self.signal_lock_button,            21, 0)
        parameters_layout.addWidget(self.signal_lock_ON_button,         21, 1)
        
        parameters_layout.addWidget(self.fit_poly_option,               22, 0)
        
        parameters_layout.addWidget(n_polynomial_Label,                 23, 0)
        parameters_layout.addWidget(self.n_polynomial_Edit,             23, 1)
        
        parameters_layout.addWidget(self.fit_raman_option,              24, 0)
        
        parameters_layout.addWidget(center_row_bkg_Label,               25, 0)
        parameters_layout.addWidget(self.center_row_bkg_Edit,           25, 1)
    ...
    @pyqtSlot(np.ndarray, np.ndarray)
    def spectrum_line_show_s(self, array_wavelength, array_profile):
    ...
    def get_lock_signal(self):
        ...
        if self.signal_lock_button.isChecked():
           self.signal_lock_Signal.emit(center_wavelength, exposure_time)
           
    def get_lock_signal_ON(self):
        if self.signal_lock_ON_button.isChecked():
            self.signal_lock_ON_Signal.emit(True)
        else:
            self.signal_lock_ON_Signal.emit(False)
    ...
    def make_connection(self, backend):
        ...
        backend.line_spectrumSignal_s.connect(self.spectrum_line_show_s)
...
class Backend(QtCore.QObject):
    ...
    line_spectrumSignal_s = pyqtSignal(np.ndarray, np.ndarray)
    ...
    def __init__(self, myAndor, mySpectrometer, *args, **kwargs):
        ...
        self.sustrate_signal_bool = False
        self.signal_sustrate = np.zeros(1002)
    ...
    def line_spectrum_parameters(self, center_row, spot_size, center_bkg):
        ...
        if not self.normalized_lamp_bool:
            ...
            if self.sustrate_signal_bool:
                
                spectrum_NP_s = spectrum_NP - self.signal_sustrate 
                self.line_spectrumSignal_s.emit(wavelength, spectrum_NP_s)
            else:
                spectrum_NP_s = spectrum_NP
            
            if self.fit_raman_water:
                
                try:
                
                    fit_wavelength, fit_spectrum_NP, best_parameters = fit_signal_raman(wavelength, spectrum_NP_s, self.lower_wavelength, self.upper_wavelength)
            ...
            if self.fit_poly:
                   ...
                   fitpoly_wavelength, fitpoly_spectrum_NP, max_wavelength = fit_signal_polynomial(wavelength, spectrum_NP_s, self.lower_wavelength, self.upper_wavelength, self.npol)
    ...
    @pyqtSlot(float, float)
    def taking_signal_sustrate(self, Wavelength, exposure_time):
       ...
       self.signal_sustrate = spectrum
       
       timestr = time.strftime("%Y%m%d-%H%M%S")
       new_folder = os.path.join(self.file_path,  timestr + "_" + "Spectrum_Sustrate")
       os.makedirs(new_folder)
       self.new_folder = new_folder
       self.save_calibration_spectrum(self.calibration, self.step)
       self.save_picture_spectrum(self.image, self.step)
       self.save_line_spectrum(wavelength, spectrum, spectrum_bkg, self.step)
       
       print(datetime.now(), '[PySpectrum]', 'Lock spectrum on sustrate')
       
       return
           
    @pyqtSlot(bool)
    def signal_sustrate_ON(self, Savebool):     
           
        if Savebool:
            
             self.sustrate_signal_bool = True 
             print(datetime.now(), '[PySpectrum]', 'Spectrum on sustrate ON')
            
        if not Savebool:
            
            self.sustrate_signal_bool = False
            print(datetime.now(), '[PySpectrum]', 'Spectrum on sustrate OFF')
  ...
  #  def close(self):
    ...
    def make_connection(self, frontend):
        ...
        frontend.signal_lock_Signal.connect(self.taking_signal_sustrate)
        frontend.signal_lock_ON_Signal.connect(self.signal_sustrate_ON)
